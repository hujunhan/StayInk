...
- Classification: INFERENCE
- Kindle model / firmware: observed Scribe / 5.19.5 for the Phase 4A design; other firmware UNKNOWN
- Confidence: low
- Status: Phase 4B host guard/core audit passed; execution not authorized pending an audited ARM no-write binary and both target no-write dry runs
...
8. **Smallest safe test:** `docs/research/HIDE_METADATA_EXPERIMENT.md` defines the bounded trial and explicitly withholds execution authorization. `docs/research/GUARD_WATCHDOG_VALIDATION.md` records passing host-core/source audit and the remaining gate: produce and audit the ARM no-write binary, then separately authorize the ordinary and detached-watchdog target no-write runs. The eventual separately authorized trial may change only `WM_NAME`, must have exact and timed restore plus independent operator recovery, and must preserve ordinary powerd lifecycle. Success additionally requires the original Notebook image to remain visible, control-equivalent suspend statistics/kernel PM evidence, normal physical wake, and full Notebook restoration.
