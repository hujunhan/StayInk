...
2. **Supporting evidence:** FACT-003 provides an observable interval between screen-saver entry and readiness on one Scribe; FACT-005 shows memory updates and panel refresh are separate; FACT-006 shows Scribe-specific MTK support in two projects; FACT-010 makes passive persistence plausible.
...
2. **Supporting evidence:** FACT-012 shows KOReader can unload/reload a blanket screensaver module in a conditional launch mode; FACT-003 separates screen-saver entry from readiness on one Scribe; SRC-009 reports lock continuing without visible replacement when modified stock resources failed to resolve.
3. **Assumptions:** the same blanket module exists and owns replacement on target Scribe firmware; its runtime control does not gate readiness; unloading is transient; reload is sufficient for normal wake; Special Offers/passcode variants do not use a different actor.
4. **Unknowns:** module presence, ownership, accepted values, internal dependencies, timing, persistence, and behavior across firmware and Scribe generations.
