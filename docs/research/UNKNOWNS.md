...
- Resolution criteria: design—but do not execute without separate approval—a bounded title-metadata trial with one changed variable, exact title restore, an independent timed/operator recovery path, pre/post suspend evidence, normal physical wake, and verified Notebook restoration. The design must specify how it will detect and recover from unexpected Awesome, Expose, focus, or wake behavior.
