...
- Classification: UNKNOWN
- Sources: OBS-014, OBS-016, OBS-017
- Model / firmware: UI-identified Kindle Scribe, generation UNKNOWN / 5.19.5 for the copied artifacts; other applicability UNKNOWN
...
- Current evidence: FACT-029 identifies a narrow X11 map/raise helper, but its caller ignores the helper return and marks the plugin presented. Mapping also triggers an Expose path containing deferred rendering and flash-trigger work. Normal exit conditionally unmaps based on that internal state and always attempts two exit notifications. Blanket's existing whole-module unload removes LIPC callbacks, invokes deinit, destroys the window/Cairo/content state, closes the object, and frees loader state. No reviewed `uiQuery`, loader, plugin, or window-manager interface controls only `blanket_screensaver` visibility while preserving coherent plugin state.
- Resolution criteria: identify a stock, runtime-only, non-patching interface that can withhold and restore only the window/layer presentation while keeping plugin state and required exit notifications coherent; correlate its read-only state with visible presentation; and define a bounded restore path before any state-changing trial.
